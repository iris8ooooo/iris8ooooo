...
- [ ] **M1: 스캐폴드 + 달력 + 프리셋/커스텀 공수 입력 + 월 합계** ← 진행 중
- [ ] M2: 업체 관리 + 단가 이력 + 색상 표시 + 부가항목
...
## 백로그 (리뷰에서 나왔으나 보류한 항목 — M6 마감 전 처리)

- 다크모드에서 마커 팔레트 일부 색(남색·보라·갈색·회청·진빨강)의 셀 점 대비가 약함 → MarkerPalette에 다크모드 변형 색 추가 (M6 다크모드 마감 때)
- 메모 작성 중 시트 드래그/스크림 탭으로 닫으면 작성 내용 무경고 유실 → 메모 모드에서 드래그 잠금 또는 자동 임시저장 (M6)
- 최근 커스텀 입력값을 프리셋 그리드 끝에 임시 칩으로 노출 (설계 심사 graft 제안, M2 검토)
- 위젯 테스트에서 drift 스트림을 FakeAsync 안에서 직접 await하면 언마운트가 영구 행 — DB 검증은 `getRange` 같은 일반 쿼리 Future만 사용할 것 (재발 주의)
