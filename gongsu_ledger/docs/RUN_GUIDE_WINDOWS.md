# 공수수첩 실행 가이드 (비개발자용, Windows PC + 갤럭시 기준)

Windows PC에서 공수수첩 앱을 갤럭시(또는 가상 안드로이드 폰)에 띄워 보는 방법입니다.
...
- 폰에 **공수수첩**(첫 실행이면 직군 선택 화면)가 뜨면 성공. PowerShell에서 `q` 를 누르면 끝납니다.
...
- [ ] 홈 화면에 **공수수첩** 이름과 파란 달력 아이콘이 보인다
- [ ] 홈 위젯: 홈 화면 빈 곳 길게 누르기 → **위젯** → **공수수첩** → "이번 달 공수"(2×2)를 끌어다 놓기. 무료 상태면 "프로 전용" 문구, 프로면 이번 달 공수·실수령이 보인다
