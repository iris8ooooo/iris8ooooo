# 공수수첩 실행 가이드 (비개발자용, Mac 기준)

이 문서는 개발 지식 없이 Mac에서 공수수첩 앱을 직접 실행해 보는 방법입니다.
...
**가장 쉬운 방법 — 바탕화면 아이콘 만들기 (한 번만)**

터미널에 아래 한 줄을 붙여넣고 엔터를 치세요. 바탕화면에 **공수수첩** 아이콘이 생깁니다.

```bash
~/development/gongsu-app/gongsu_ledger/tool/mac/install_desktop_shortcut.command
```

앞으로는 **바탕화면의 "공수수첩" 를 두 번 누르기만** 하면 됩니다. 최신 코드 받기 → 준비 → 시뮬레이터 켜기 → 실행까지 알아서 진행합니다. 폴더를 찾아 들어갈 필요가 없습니다.

> Finder 에서 직접 찾으려면: 메뉴 막대의 **이동 → 홈**(⇧⌘H)을 눌러야 `development` 폴더가 보입니다. Finder 는 홈 폴더를 기본으로 보여주지 않습니다.
...
첫 실행은 몇 분 걸립니다. 시뮬레이터에 **공수수첩 달력**이 뜨면 성공입니다.
...
## 먼저 — 오너가 직접 볼 것은 3가지뿐입니다

아래 M1~M6 체크리스트는 전부 훑어보면 좋지만, **대부분은 자동 검사(`flutter test` 220개)가 이미 증명**하고 있습니다. 사람 눈이 꼭 필요한 건 이 셋입니다.

1. **홈 위젯 추가** (M5) — 시뮬레이터 홈에서 빈 곳 길게 누르기 → 왼쪽 위 **+** → "공수수첩" → 위젯 추가. 앱과 위젯이 데이터를 나눠 보는 부분이라 기계가 대신 못 봅니다.
2. **글씨 "아주 크게" 로 바꿔 보기** (M6, 설정) — 화면이 깨지는 곳이 없는지. 주 사용층이 40~60대라 이게 중요합니다.
3. **손에 쥐고 써 보기** — 날짜 탭 → 공수 버튼. 2탭 안에 끝나는지, 손가락에 걸리는 데가 없는지. 숫자가 맞는지는 기계가 보지만, 쓰기 편한지는 사람만 압니다.

이미 자동으로 증명된 것들 (직접 안 하셔도 됩니다):

- **1.8 공수 반올림 없음** — 테스트가 실제 키패드 버튼을 눌러 1·.·8 을 입력하고 DB 에 180(=1.8공수)으로 저장되는지 확인합니다
- **비행기 모드 전 기능** — 앱 코드에 네트워크를 쓸 수단이 아예 없고(테스트가 증명), 출시용 안드로이드 빌드는 인터넷 권한조차 요구하지 않습니다
- **광고·추적 SDK 없음** — 의존성 목록에 광고/분석 패키지가 들어오면 테스트가 실패합니다
- 세금 계산, 단가 이력, 백업 왕복, DB 마이그레이션 — 전부 단위 테스트
...
- [ ] 날짜를 탭하면 아래에서 시트가 올라오고(맨 위에 "14일  9월 · 월요일"), **1 타일 한 번**으로 입력 완료 (총 2탭)
- [ ] 달력 위 **큰 숫자**(이 달 총 공수)와 아래 "근무 n일"이 바로 갱신되고, 입력한 날짜 칸이 옅은 남색으로 칠해진다(공수가 많을수록 진해진다: 0.5 → 1 → 1.5 → 2 이상)
- [ ] 기록이 있는 날은 시트가 안 닫혀서 **잔업·반공을 연달아 추가**할 수 있다. 이미 넣은 프리셋 타일은 남색으로 채워져 보인다. **닫기** 버튼으로 내린다
- [ ] 점선 타일 **직접 입력**에서 자체 키패드로 `1.8` 입력 → 정확히 1.8로 저장 (반올림 없음!)
- [ ] 시트 아래 **연속 채우기** → "이 달 말까지" 그대로 **채우기** → 오늘부터 말일까지 일요일·공휴일을 뺀 날에 1공수가 한 번에 들어가고 "n일에 1공수 기록을 넣었어요" 안내가 뜬다. 이미 기록이 있는 날은 건드리지 않는다
- [ ] `1.33`처럼 0.05 단위가 아니면 저장 버튼이 눌리지 않고 안내가 뜬다
- [ ] 기록을 삭제하면 **실행 취소**가 뜨고, 눌러 보면 되살아난다
- [ ] 시트 오른쪽 위 **메모**를 눌러 메모를 남기면 달력 칸에 작은 남색 네모가 생긴다
...
- [ ] 날짜 탭 → 시트 맨 위에 **업체 알약**이 생겼다(고른 것은 남색). 업체를 고르고 프리셋 탭 → 달력 점이 **업체 색**으로 표시
- [ ] 달력 위 큰 숫자가 **예상 수입 (세전)** 금액으로 바뀐다 (1공수 × 150,000원)
...
- [ ] 메뉴(⋮) → **업체(현장) 관리** → 업체 탭 → **세금 방식**에서 "3.3% 원천징수" 선택 → 저장
  - 달력 위 큰 숫자가 **실수령**으로 바뀌고 세전보다 3.3%만큼 적다 (예: 300,000원 → 290,100원). 그 아래 "세전 300,000원 · 세금·보험 공제 9,900원"
...
- [ ] 달력에서 **공휴일**(예: 10월 3일 개천절, 10월 9일 한글날) 칸에 빨간 테두리와 이름이 보이고 숫자가 빨간색이다. 달력 위 오른쪽에 "개천절 3 · 대체휴일 5 · 한글날 9" 요약, 큰 숫자 아래에 "공휴일 n일"이 빨갛게 보인다. 그 날짜를 탭하면 시트 머리에 "10월 · 토요일 · 개천절"처럼 공휴일 이름이 빨갛게 붙는다
...
- [ ] 달력 화면 오른쪽 위 **공유 아이콘** → 이번 달 달력(달 이름·큰 숫자·범례 포함, 버튼 제외)이 이미지로 공유된다
...
- [ ] 시뮬레이터 홈 화면(앱 밖)에서 빈 곳을 길게 누르기 → 왼쪽 위 **+** → 검색창에 "공수수첩" → **이번 달 공수** 위젯 → **위젯 추가**
...
**갤럭시(안드로이드)에서 보려면**: Windows PC에 Flutter를 설치하고 USB로 연결해 `flutter run` (이 가이드는 Mac 기준이라, Windows용은 `RUN_GUIDE_WINDOWS.md`를 보세요). 홈 화면 길게 누르기 → **위젯** → **공수수첩** → "이번 달 공수"(2×2)를 끌어다 놓기.
...
- [ ] 앱 이름이 홈 화면에 **공수수첩**로, 아이콘이 **종이 바탕에 남색 막대 + 12칸 격자(금색 한 칸)** 로 보인다
