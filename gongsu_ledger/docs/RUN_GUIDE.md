...
터미널에 아래 한 줄을 붙여넣고 엔터를 치세요. 바탕화면에 **공수장부** 아이콘이 생깁니다.

```bash
~/development/gongsu-app/gongsu_ledger/tool/mac/install_desktop_shortcut.command
```

앞으로는 **바탕화면의 "공수장부" 를 두 번 누르기만** 하면 됩니다. 최신 코드 받기 → 준비 → 시뮬레이터 켜기 → 실행까지 알아서 진행합니다. 폴더를 찾아 들어갈 필요가 없습니다.
...
1. **홈 위젯 추가** (M5) — 시뮬레이터 홈에서 빈 곳 길게 누르기 → 왼쪽 위 **+** → "공수장부" → 위젯 추가. 앱과 위젯이 데이터를 나눠 보는 부분이라 기계가 대신 못 봅니다.
